...
do #for all of the files in conversion area
	...
	if [ "$genNor" == "Ready" ] ; then
		...
		#genNorSampleList=`cat $fqList | awk 'BEGIN{FS=","} $7=="Genome" && $9=="Constitutional" {print $5}' | uniq | xargs`
	...
	if [ "$genTum" == "Ready" ] ; then
		...
		#genTumSampleList=`cat $fqList | awk 'BEGIN{FS=","} $7=="Genome" && $9=="Tumor" {print $5}' | uniq | xargs`
	...
	if [ "$exoNor" == "Ready" ] ; then
		...
		#exoNorSampleList=`cat $fqList | awk 'BEGIN{FS=","} $7=="Exome" && $9=="Constitutional" {print $5}' | uniq | xargs`
	...
	if [ "$exoTum" == "Ready" ] ; then
		...
		#exoTumSampleList=`cat $fqList | awk 'BEGIN{FS=","} $7=="Exome" && $9=="Tumor" {print $5}' | uniq | xargs`
	fi
	...
	do
		...
		for eachRead in `cat $fqList | awk 'BEGIN{FS=","} $5=="'"$eachSample"'" {print $1","$2}'`
		do
